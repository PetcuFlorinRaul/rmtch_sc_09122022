...
function EditPage() {
  ...
  async function getPageData(url: string, pageID: number) {
    ...
    .then((res) => {
        ...
        setValue(res.data.pageContent)
        setSubPagesState(res.data.subpagesEnabled)
    })
    .catch((err) => {
        console.log(err)
    })
  ...
  async function updatePage(url: string, data: BodyData) {
  ...
  async function createPage(pageName: string, value: string, subpagesState: boolean) {
    ...
    if(Object.keys(errors).length > 0) {
      ...
      await updatePage('http://localhost:5000/pages/modify_page', {
        pageID: truePageID,
        pageName: pageTitle,
        pageContent: value,
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {
              loading ? <p>Loading...</p> : <p>Update page</p>
            }
          </motion.div>
        </div>
      </div>
